...
def defend_side(new, board, symbol):
    ...
    for side in sides:
        ...
        if new in side and new not in corner_diags:
            ...
            for field in side:
                ...
                if field not in corners: fields.append(field)
    ...
    for side in sides:
        for field in side:
            ...
            if field not in corners:
                ...
                if adjacent >= 2 and threatened == 0:
...
def second_turn(new, board, symbol):
    ...
    ret = defend_side(new, board, symbol)
    if ret is not None: return ret
...
def turn(board, symbol):
    ...
    if turn_counter == 1:
        ...
        if turn_counter == 32: print("Finished")
