...
# ! If Sides Are Filled, Do Not Do Corner Kills
# x Test Corner Placements (Outplay)
#
#
#
#
...
def corner_side(board, symbol):
    ...
    # if getboard(board, choice[0], choice[1]) == '#': return choice[0], choice[1]
...
def defend_side_ng(new, board, symbol):
...
def fill_middle_ng(board, symbol):
    ...
    return rating(board, symbol, middle)  # if len(choices) != 0: return rating(board, symbol, choices)
...
def rndm(board):
    ...
    while True:
        ...
        if (x, y) not in corners and (x, y) not in corner_diags:
...
def second_turn(new, board, symbol):
    ...
    ret = closed_corner(new, board)
    if ret is not None: return ret

    # If Enemy At Side, Then Corner Side At Same Side
    ret = defend_side(new, board)
    if ret is not None: return ret
...
def random_turn(new, board, symbol):
    ...
    ret = defend_side_ng(new, board, symbol)
    if ret is not None: return ret
    ...
    """
    # If Sides Are Filled Place Corners
    ret = rating(board, symbol, corners)
    if ret is not None: return ret
    """
    ...
    """
    # Else Error, But Do Not Break The Battle
    ret = rndm(board)
    if ret is not None: print("error"); return ret
    """
...
def turn(board, symbol):
    ...
    for y in range(8):
        for x in range(8):
            ...
            if my_board[y][x] != old_board[y][x]:
                if my_board[y][x] != symbol and old_board[y][x] != symbol:
                    new = (x, y); break
    ...
    if turn_counter == 1:
        ...
        ret = random_turn(new, board, symbol)
