...
# TODO:
# Try To Kill Threats, Do Not Protect
# Check Injections in Middle
#
#
#
...
def rating(board, symbol, choices):
    ...
    try:
        ...
        if len(rating_dict[ratings[-1][0]][7]) == 1:

            possibilities = []
            enemy = rating_dict[ratings[-1][0]][7][0]

            for element in choices:

                board_copy = flip_rating(board, symbol, element[0], element[1])

                for x in range(8):
                    for y in range(8):

                        if (x, y) == enemy:
                            if getboard(board, x, y) != getboard(board_copy, x, y):
                                possibilities.append(element)

            reratings = rerating(board, symbol, possibilities)

            try:
                reratings = sorted(reratings, key=lambda x: x[1])

                if reratings[-1][1] >= ratings[-1][1]:
                    print(turn_counter, "Improvement!")
                    return reratings[-1][0]
...
def rerating(board, symbol, choices):
    ...
    for element in choices:
        ...
        if getboard(board, x, y) == "#":
            ...
            recalculated_rating = recalculate_rating(kills, enemy_kills, threatened,
                                                 adjacent_ones, in_corner_side)

            rerating_dict.update({(x, y): (recalculated_rating, kills, enemy_kills,
                                         threatened, adjacent_ones, in_corner_side)})

            reratings.append(((x, y), recalculated_rating))
...
def recalculate_rating(kills, protected, threatened, adjacent_ones, in_corner_side):

    score = (1.5 * kills + 0.75 * protected + -1 * threatened + 0.5 * adjacent_ones
             + 0.5 * in_corner_side)
